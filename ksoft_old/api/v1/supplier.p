...
PROCEDURE pi-get:
    ...
    DEF VAR lCarrier        AS LOGICAL NO-UNDO .
    ...
    ASSIGN lCarrier = NO .
    IF emitente.cgc <> "" THEN DO:
        FIND FIRST transporte NO-LOCK
            WHERE transporte.cgc = emitente.cgc
            NO-ERROR .
    ...
    oOut:ADD("is_carrier"       , lCarrier) .
    oOut:ADD("id_carrier"       , IF AVAIL transporte THEN transporte.cod-transp ELSE 0) .
END.
...
PROCEDURE pi-post :
    ...
    /* Criacao do regitro de Transportadora, verificado na BOAD268 */
    DEF VAR i-cod-transp    AS INT NO-UNDO .

    ASSIGN oIn = fnApiReadBody(oIn) .

    FIND FIRST emitente NO-LOCK
        WHERE emitente.nome-abrev = fnApiGetChar(oIn,"nome_abrev")
        NO-ERROR .

    IF AVAIL emitente AND
       emitente.cgc <> "" AND
       fnApiGetLogical(oIn,"create_carrier") = YES
    THEN DO:
        FIND FIRST transporte NO-LOCK
            WHERE transporte.cgc = emitente.cgc
            NO-ERROR .
        ...
        IF NOT AVAIL transporte THEN DO:
            /* Achar prox ID livre */
            ASSIGN i-cod-transp = 1000 .
            REPEAT:
                FIND FIRST transporte NO-LOCK
                    WHERE transporte.cod-transp = i-cod-transp
                    NO-ERROR .
                IF NOT AVAIL transporte THEN LEAVE .
                ASSIGN i-cod-transp = i-cod-transp + 1 .
            END.
            TRA1:
            DO TRANSACTION ON ERROR UNDO , LEAVE
                :
                CREATE transporte . ASSIGN
                    transporte.cod-transp       = i-cod-transp
                    transporte.nome             = emitente.nome-emit
                    transporte.nome-abrev       = emitente.nome-abrev
                    transporte.cod-emitente     = emitente.cod-emitente
                    transporte.cgc              = emitente.cgc
                    transporte.natureza         = emitente.natureza
                    transporte.endereco         = emitente.endereco
                    transporte.bairro           = emitente.bairro
                    transporte.cidade           = emitente.cidade
                    transporte.estado           = emitente.estado
                    transporte.cep              = emitente.cep
                    transporte.pais             = emitente.pais
                    transporte.ins-estadual     = emitente.ins-estadual
                    transporte.ins-municipal    = emitente.ins-municipal
                    transporte.contato          = emitente.contato[1]
                    transporte.telefone         = emitente.telefone[1]
                    transporte.via-transp       = 1 /* Rodoviario */
                    transporte.zip-code         = emitente.zip-code
                    transporte.e-mail           = emitente.e-mail
                    .
                FIND CURRENT transporte NO-LOCK .
            END.
        END.
    END.
