...
DEF VAR dSaldo      AS DECIMAL NO-UNDO .
...
FOR EACH ITEM NO-LOCK
    ...
    FOR EACH saldo-estoq NO-LOCK
        ...
        :
        ASSIGN dSaldo = saldo-estoq.qtidade-atu .
        FOR EACH movto-estoq NO-LOCK OF saldo-estoq
            WHERE movto-estoq.dt-trans > DATE("30/11/2025")
            :
            IF movto-estoq.tipo-trans = 1 THEN DO:
                ASSIGN dSaldo = dSaldo - movto-estoq.quantidade .
            END.
            ELSE DO:
                ASSIGN dSaldo = dSaldo + movto-estoq.quantidade .
            END.
        ...
        IF NOT AVAIL cst-prod THEN DO:
            FIND FIRST cst-prod NO-LOCK
                WHERE cst-prod.nr-ord-produ     = INT(SUBSTRING(cLoteOP, 1, 8))
                AND   ROUND(cst-prod.m_quadrados, 0) = ROUND(dSaldo, 0)
                AND   cst-prod.nr-reporte <> 0
                NO-ERROR .
        END.
        IF NOT AVAIL cst-prod THEN DO:
        ...
        FIND LAST zz-etiqueta NO-LOCK
            WHERE zz-etiqueta.lote          = saldo-estoq.lote
        ...
        PUT UNFORMATTED
            ...
            ';' IF AVAIL cst-prod THEN ROUND((dSaldo / cst-prod.m_quadrados ) * cst-prod.qt-folhas, 0) ELSE 0 //Qt in PSC
