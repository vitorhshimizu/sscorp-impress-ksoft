...
PROCEDURE pi-get:
    ...
    oOut:ADD("aliquota_ipi"     , ITEM.aliquota-ipi) .
    ...
    oOut:ADD("n_fci"            , c-chave-fci) .
    oOut:ADD("dt_fci"           , IF AVAIL inf-compl THEN inf-compl.dat-campo ELSE ?) .
END.
...
PROCEDURE pi-post:
    ...
    FIND FIRST classif-fisc NO-LOCK
        ...
        NO-ERROR .
    IF (NOT AVAIL classif-fisc AND fnApiGetLogical(oIn, "ind_item_fat") = YES ) OR
       (NOT AVAIL classif-fisc AND fnApiGetChar(oIn, "ncm") <> "" )
    THEN DO:
    ...
    ASSIGN
        ...
        tt-item.aliquota-ipi            = IF AVAIL classif-fisc THEN classif-fisc.aliquota-ipi ELSE 0 
        ...
        tt-item.tipo-contr              = IF AVAIL ITEM THEN item.tipo-contr ELSE 4 /* Debito Direto CE0108 */
        tt-item.tipo-con-est            = IF AVAIL ITEM THEN item.tipo-con-est ELSE 1 /* Serial CE0111 */
        .
    ...
    DO TRANSACTION ON ERROR UNDO , LEAVE ON STOP UNDO , LEAVE
        ...
        ASSIGN 
            ...
            ITEM.aliquota-ipi       = IF AVAIL classif-fisc THEN classif-fisc.aliquota-ipi ELSE 0
