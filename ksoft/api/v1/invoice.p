...
PROCEDURE pi-post-manual-ft4003:
    ...
    THEN DO:
        ...
        /* Envia nota via TSS */
        IF nota-fiscal.idi-sit-nf-eletro = 1 /*Nao Gerada*/ AND 
           //NOT natur-oper.log-envio-xml-nf-eletro-manual
           SUBSTRING(natur-oper.char-1,158,1) <> "S"
        THEN DO:
...
PROCEDURE pi-get-nfe-xml:
    ...
    */
    ...
    /* Retornar XML da pasta Historico */
    /* APENAS DEVIDO AO ERRO NO SEFAZ - AGUARDANDO ATUALIZACAO DA TOTVS */
    /*
...
PROCEDURE pi-cancel
    ...
    oIn:WriteFile("C:\totvs\ksoft_log\POST_invoice_cancel\" + fnNowToString() + ".json", YES, "UTF-8") .

    /* Cancelamento de Importacao */
    IF fnApiGetChar(oIn, "motivo_cancelamento") MATCHES "*T210*" THEN DO:
        oOut = fnApiOK() .
        oOut:ADD("cod_estabel"  , fnApiGetChar(oIn, "cod_estabel") ) .
        oOut:ADD("serie"        , fnApiGetChar(oIn, "serie") ) .
        oOut:ADD("nr_nota_fis"  , fnApiGetChar(oIn, "nr_nota_fis") ) .
        RETURN .
    END.
